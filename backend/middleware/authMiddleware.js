...
const protect = asyncHandler(async (req, res, next) => {
  ...
  if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) {
    try {
      ...
      token = req.headers.authorization.split(" ")[1]; // turn into an array where the text 'bearer' is the first item and token is second item

      // verify token
      const decoded = jwt.verify(token, process.env.JWT_SECRET);

      // Get user from the id that's in the token (-password ensures the password is not included)
